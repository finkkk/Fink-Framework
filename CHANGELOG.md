...
- 存档可靠性与恢复机制完善：采用临时文件回读校验和原子替换，自动维护即时备份与编号历史备份；主档损坏时会依次尝试 Backup / History，并保留旧版目录结构读取兼容。
- 存档格式复用数据管线：JSON 模式输出可直接查看的 UTF-8 文件，Binary 模式复用 Odin、AES 密钥及可选 GZip 压缩；存档格式和二进制后缀可在独立设置页配置。
- 存档 Schema 兼容能力补全：新增 `FormerSaveNames` 成员重命名映射，Binary 旧档加载时执行构造函数与字段初始化器，使新增字段自然获得当前默认值，并修复 decimal 极值及多种数值边界的往返问题。
- 存档系统收尾加固：修复槽位删除遗漏 `_bak` 历史文件、旧版历史目录无法枚举/回档、恢复来源标记错误，以及提交阶段错误状态过度归类的问题；补齐核心公共 API 文档。
