...
namespace FinkFramework.Runtime.UI.Canva
{
    ...
    public class CanvasManager : Singleton<CanvasManager>
    {
        ...
        {
            rootDic.Clear();
            var roots = Object.FindObjectsOfType<CanvasRoot>(true);
        ...
        private void RegisterCanvas(CanvasRoot canvasRoot)
        {
            var canvas = canvasRoot.GetComponent<Canvas>();
            if (!canvas)
            {
                LogUtil.Warn($"{canvasRoot.name} 没有 Canvas 组件，已忽略");
                return;
            }
            // 兜底：如果没有 panelParent，就用 canvas 自身
            var parent = canvasRoot.panelParent ? canvasRoot.panelParent : canvas.transform;
            // 兜底：如果没填 Id，就用对象名
            var localId = string.IsNullOrEmpty(canvasRoot.canvasId) ? canvasRoot.name : canvasRoot.canvasId;
            // 拼接场景名，避免不同场景冲突
            string sceneName = canvas.gameObject.scene.name;
            string finalId = $"{sceneName}_{localId}";
            var key = (canvasRoot.rootType, finalId);
            // 自动设置 worldCamera
