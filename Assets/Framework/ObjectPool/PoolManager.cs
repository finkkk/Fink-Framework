...
namespace Framework.ObjectPool
{
    ...
    public class PoolManager : Singleton<PoolManager>
    {
        ...
        public GameObject Spawn(string name)
        {
            ...
            GameObject obj;
            // 1.如果全局对象池中不存在该对象池
            if (!poolDic.TryGetValue(name, out var value))
            {
                // 动态创建对象
                obj = Object.Instantiate(ResManager.Instance.Load<GameObject>(name));
                if (!obj)
                {
                    LogUtil.Error($"资源路径 {name} 无法加载，请检查路径是否正确！");
                    return null;
                }
        ...
        public T Spawn<T>(string nameSpace = "") where T : class,IPoolObject,new()
        {
            ...
            {
                PoolObject<T> pool = value as PoolObject<T>;
                // 若池子不为空
        ...
        /// </summary>
        public void Despawn<T>(T obj,string nameSpace = "") where T : class,IPoolObject
        {
            if (obj == null)
            {
                return;
